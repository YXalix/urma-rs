...
for pair in "a:nodeB" "b:nodeA"; do
    log=${pair%%:*}
    peer=${pair##*:}
    line="read 64 bytes via $VIA: \"hello world from $peer\""
...
if [ "$FAIL" -eq 0 ]; then
    echo "PASS: both nodes read the peer's message via $VIA"
else
